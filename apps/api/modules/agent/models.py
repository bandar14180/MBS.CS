...
from sqlalchemy import Float, ForeignKey, func, Integer, String, Text, text, UniqueConstraint
...
class EngagementState(Base):
    ...
    Tenant-scoped by its own workspace_id: a DIRECT table in apps/api/core/tenancy.py, so
    every ORM query is auto-filtered on workspace_id (scans is in EXEMPT_TABLES, so we carry
    our own column and scope directly, like ai_usage/attack_narratives)."""
    ...
    approval_state: Mapped[dict] = mapped_column(JSONType, nullable=False, default=dict)
    # Accumulated attack graph (nodes/edges: hosts, services, findings, access).
    attack_graph: Mapped[dict] = mapped_column(JSONType, nullable=False, default=dict)
...
class AgentStep(Base):
    ...
    its own workspace_id (a DIRECT table in tenancy.py's auto-filter)."""
...
class AgentDecision(Base):
    ...
    as a DIRECT table in tenancy.py, same pattern as agent_steps / attack_narratives."""
    ...
    agent_step_id: Mapped[uuid.UUID | None] = mapped_column(
        GUID(), ForeignKey("agent_steps.id", ondelete="SET NULL"), nullable=True
    )
    step_no: Mapped[int] = mapped_column(Integer, nullable=False)
    phase: Mapped[str] = mapped_column(String(32), nullable=False)
    # run_tool | finish
    ...
    observations: Mapped[list] = mapped_column(JSONType, nullable=False, default=list)
    inferences: Mapped[list] = mapped_column(JSONType, nullable=False, default=list)
    hypotheses: Mapped[list] = mapped_column(JSONType, nullable=False, default=list)
    ...
    candidates: Mapped[list] = mapped_column(JSONType, nullable=False, default=list)
    ...
    budget_state: Mapped[dict] = mapped_column(JSONType, nullable=False, default=dict)
