...
- **Planner is opt-in in the orchestrator; default off = unchanged behavior.** Only when `scan.config.use_ai_planner` is set does the orchestrator call the planner (and persist an `ai_plans` row: sequence + reasoning + `model_version` + `prompt_version`); otherwise it uses the deterministic phase order from step 5. Phase order still governs actual pipeline dependencies even when the AI reorders. `GET .../scans/{id}/ai-plan` reads the plan (404 when AI planning was off — verified live).
