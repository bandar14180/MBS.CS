...
## Step 6 Implementation Notes (2026-07-27)

The Vulnerability Engine, the first vuln-producing tool (**Nuclei**), and the vulnerability lifecycle API are built — and this is where the **active-testing gate** (§7) finally gets enforced, because Nuclei is the first tool that sends payloads. Modules: `apps/api/modules/vulnerabilities`, `apps/api/scanner_engine/tool_runners/nuclei_runner.py`. Migrations `b41fcdb0de77` (tables + RLS) and `bbcddc51e96a` (permission seed). Verified live end-to-end against `nginx`: an active scan produced 10 findings; a rescan **deduped** (stayed 10) while **appending a second evidence link** to each; a `false_positive` status set by an analyst **stuck** across the rescan.

- **Assets vs vulnerabilities are separate outputs.** Rather than overload `CommonFinding`, the runner contract gained a second, optional method: `parse_vulnerabilities(raw) -> list[VulnerabilityFinding]` (base default `[]`). Recon runners and their tests are untouched; only Nuclei overrides it (and returns `[]` from `parse()` since it inventories nothing). The orchestrator calls both and routes assets → `upsert_asset`, vulns → the Vulnerability Engine.
- **Dedup by fingerprint, lifecycle by rule.** A vuln's identity is `(project_id, fingerprint)` where Nuclei's fingerprint is `template-id|matcher-name|matched-at` — stable across scans. New fingerprint → `open`; recurring → refresh metadata + `last_seen_scan_id`, and if it was `fixed` it flips to `reopened` (regression). Analyst decisions (`false_positive`, `accepted_risk`) are sticky — the engine never resurrects them on re-detection. No auto-fix-on-absence (that needs careful scope reasoning to avoid false "fixed"; deferred).
- **Evidence linkage is mandatory (§1).** Every ingest appends a `vulnerability_evidence` row tying the vuln to the tool-run's evidence artifact; a rescan adds another, so a finding accumulates its evidence trail (verified: 2 links after 2 scans).
- **Active-testing gate, enforced twice.** `NucleiRunner.requires_active_testing = True`. Scan creation 403s if any requested module requires active testing and the target's scope has `active_testing_allowed = False` (checked off the registry class attr, no runner instantiation). The orchestrator re-checks at execution and records a visible `skipped_unauthorized` tool_run if authorization was revoked between queue and run. All prior recon tools stay passive.
- **Nuclei runs last (phase 50), on httpx's http services.** It reads `http_service` findings from the pipeline (falls back to `http(s)://<resolved-target>`), feeds URLs via stdin, and runs `-tags misconfig` by default (focused/fast; reliably fires e.g. missing-security-headers on a generic server — configurable via `scan.config.nuclei_tags`). The 13k-template set is **baked into the worker image** at build (`nuclei -update-templates`, no `|| true` — fail loud) so scans don't need GitHub egress at runtime, which also suits the future locked-down sandbox.
- **Vulnerability API (§6):** `GET .../vulnerabilities?severity=&status=`, `GET .../vulnerabilities/{id}`, `PATCH .../vulnerabilities/{id}/status`. Status change **requires a justification** (an auditable analyst decision) and is gated by `vulnerability:manage` (owner/admin only); `vulnerability:read` is all roles. Settable statuses: `open/confirmed/false_positive/fixed/accepted_risk` (`reopened` is engine-only).
- **Schema deviations from §5 (noted):** `vulnerabilities.asset_id` is **nullable** (best-effort link to the `http_service` asset the finding was observed at; the scan/target chain still anchors it) — plus added columns not in §5: `fingerprint` (dedup key, unique per project), `last_seen_scan_id`, and `status_justification`/`status_changed_by`/`status_changed_at` (needed because §6 mandates a justification but §5's column list predates it). CVSS uses the tool-reported score/vector when present, else a nominal severity floor (info 0.0 … critical 9.5) purely for ranking — `ai_validated` stays False (AI scoring is Phase 3).
- **Deferred:** AI correlation/dedup across *different* tools flagging the same issue (Phase 3 — current dedup is exact-fingerprint within one tool); a `GET vulnerability evidence` endpoint (linkage exists in the DB, no reader yet); auto-`fixed` on absence; SQLMap/ZAP/Nikto (later active tools).

## Step 7 Implementation Notes — AI Layer v1: Planner + Correlator (2026-07-27)

The first AI layer is built: an **AI Planner** (chooses/orders tools for a scan) and an **AI Correlator** (groups duplicate findings across tools), both calling **Claude** (`claude-opus-4-8`, the locked provider decision). Per the user's call, this was built **mockable-first**: every AI component takes an injectable Claude client, so the whole layer is unit-tested (12 tests) with a fake client and **no API key**. Live end-to-end against the real Claude API is deferred until `ANTHROPIC_API_KEY` is set in `.env`. Modules: `apps/api/ai_agent/{claude_client,planner,correlator,models}.py` + `ai_agent/prompts/`. Migration `104dc6463258` (ai_plans + RLS).

- **Provider is isolated behind one narrow interface.** `SupportsComplete.complete_json(system, user) -> dict` is the only surface the agents depend on (blueprint §3: "isolated so LLM provider/version can change without touching scan logic"). `ClaudeClient` implements it; tests inject a `FakeClient`. The client is **lazy** — importing `ai_agent` never needs a key; the key is only required when a real call is actually made, and the error message points the operator at the opt-in flags.
- **SDK-version-robust on purpose.** The wrapper uses only the classic Messages API surface (`model`/`max_tokens`/`system`/`messages`) so it works on the pinned `anthropic==0.34.2` *and* against Opus 4.8 without newer params. The system prompt demands bare JSON, which doubles as the "final-answer-only" guard Opus 4.8 wants when thinking is off. Adaptive thinking + structured outputs (`output_config`) are a documented later enhancement (bump the SDK first). `_extract_json` tolerates a ```json fence or surrounding prose.
- **The "AI never freelances" guarantee is enforced in code, not just prompted (blueprint §7).** `planner._sanitize` filters the model's proposed `tool_sequence` down to `requested_modules ∩ TOOL_REGISTRY`, drops active-testing tools unless the scope authorizes them, drops tools whose `applicable_target_types` don't match, and dedupes — so the planner can only *re-order and prune within bounds*, never introduce a tool, a command, or a payload. Unit-tested directly.
- **Planner is opt-in in the orchestrator; default off = unchanged behavior.** Only when `scan.config.use_ai_planner` is set does the orchestrator call the planner (and persist an `ai_plans` row: sequence + reasoning + `model_version` + `prompt_version`); otherwise it uses the deterministic phase order from step 5. Phase order still governs actual pipeline dependencies even when the AI reorders. `GET .../scans/{id}/ai-plan` reads the plan (404 when AI planning was off — verified live). **The flag is set via `use_ai_planner` on `POST .../scans`** (a field on `ScanCreate` that `create_scan` writes into `scan.config`). *(Fixed post-step-9: originally the orchestrator read this flag but `ScanCreate`/`create_scan` never set it, so the branch was unreachable — see the two bug-fix notes below.)*
- **AI planning is fail-soft (fixed post-step-9).** If the planner raises — no `ANTHROPIC_API_KEY`, an API error, a malformed response — the orchestrator logs a warning and **falls back to the deterministic phase order** instead of failing the whole scan; the requested tools still run. The fallback must **not** `db.rollback()`: the planner calls Claude before it writes anything, so there's nothing pending to undo, and a rollback would expire the `scan`/`target_row` ORM objects and blow up the async session with `MissingGreenlet` on the next attribute access. Verified live: `use_ai_planner=true` with no key → scan **completes** (naabu still runs), worker logs "AI planning failed … using deterministic order".
- **Correlator never invents or alters findings.** It only *groups* the input findings' ids; `AICorrelator.correlate` filters the model's groups back to the exact input id set, drops any id the model hallucinated, and emits any id the model failed to place as its own singleton group — so no finding is ever silently lost or duplicated. A single finding short-circuits with no API call. Built + unit-tested, but its **merge persistence is deliberately not wired into the orchestrator yet** — merging duplicates needs a `merged_into` relationship on `vulnerabilities` (a separate schema/design step); the service is ready to consume.
- **Prompts are versioned** (`planner/v1`, `correlator/v1` in `ai_agent/prompts/`); the version is stored on each `ai_plans` row so a plan is attributable to the exact prompt that produced it. `ai_plans` gets `ENABLE`+`FORCE` RLS via a `scans` join (scans is itself RLS-exempt — step 4).
- **Deferred:** live verification (needs a key); wiring the correlator's merges into the vuln lifecycle; the other AI Agent components from §3 (validator, fp_reducer, risk_scorer, remediation_writer, report_writer); adaptive thinking / structured outputs after an SDK bump.

## Step 9 Implementation Notes — Risk Engine + Compliance mapping (2026-07-27)

The **Risk Engine** (business risk = CVSS × asset criticality) and **Compliance Engine** (finding category → framework controls) are built and run automatically during scans. Modules: `apps/api/modules/risk`, `apps/api/modules/compliance`. Migration `ccf0f32c1190` (tables + RLS + `targets.criticality` + `target:update` permission). Verified live: an active Nuclei scan on a **high**-criticality target produced 10 `risk_scores` (CVSS × 1.5, capped at 10) and 30 `compliance_mappings` (cwe-693 → OWASP A05:2021 / NIST SC-8 / CIS 16.11).

- **Business context lives on the target.** Added `targets.criticality` (low/medium/high/critical, default medium) — the customer-set "how important is this asset" signal (§7 step 7). Settable at target creation and via `PATCH .../targets/{id}` (new `target:update` permission, owner/admin/member). This is the input that turns technical CVSS into business risk.
- **Risk is CVSS weighted by criticality, capped.** `compute_risk(cvss, criticality)` → `min(10, cvss × weight)` where weight is `{low 0.5, medium 1.0, high 1.5, critical 2.0}`, with a human-readable rationale string. `risk_scores` is one row per vuln (unique `vulnerability_id`), upserted so risk tracks the latest CVSS/context on re-detection. `GET .../vulnerabilities/{id}/risk`. (Info-severity findings score 0.0 — correct, since their CVSS is 0.0; the cap/non-zero path is unit-tested.)
- **Compliance is a curated static catalog, not a scanner (§3).** `compliance/catalog.py` maps lowercased CWE ids → `(framework, control_id, description)` tuples across owasp/nist/iso27001/pci_dss/cis. `sync_mappings` is idempotent (do-nothing on the `(vuln, framework, control)` unique key), so re-scans don't duplicate. Unknown/absent categories map to nothing (better than a wrong mapping). `GET .../vulnerabilities/{id}/compliance-mappings` (the §6 endpoint). The catalog is deliberately small but real — extend it as more finding categories appear.
- **Both run inside vuln ingestion.** The orchestrator, right after `ingest_finding` returns the vuln, calls `upsert_risk_score` (using the target's criticality it already has in scope) and `sync_mappings` (using the vuln's category). Both tables get `ENABLE`+`FORCE` RLS via a 2-hop `vulnerabilities → projects.workspace_id` join.
- **CVSS itself was already handled in step 6** (tool-reported vector/score, else a severity floor); the §7-step-8 "AI proposes a vector when tools don't supply one" remains deferred to the AI validator. Risk/compliance reads reuse `vulnerability:read`.
- **Deferred:** the Reporting Engine (§12 step 10) that renders these into exec/technical PDFs; dashboard aggregations/security-score (step 13); AI-estimated CVSS; broader compliance frameworks (SOC2/HIPAA/FedRAMP, §13).

## Step 10 Implementation Notes — Reporting Engine (2026-07-27)

The Reporting Engine renders a project's findings (severity, CVSS, business risk, compliance controls, evidence pointers) into **executive** and **technical PDF** reports. Module: `apps/api/modules/reports` (`data` gather+score, `render` reportlab PDFs, `storage` MinIO, `service`/`router`/`schemas`/`models`). Dependency: `reportlab==4.2.2` (API image rebuilt). Migration `c0ecf2f76c48` (reports table + RLS + `report:create`/`report:read` seed). Verified live: after an active Nuclei scan, both report types generate, store to the `mbs-reports` bucket, and download as valid `%PDF-` files (2.9 KB / 3.9 KB).

- **Security score.** `compute_security_score` = `max(0, 100 − Σ penalty)` over *active* findings (open/confirmed/reopened only — fixed/false-positive/accepted-risk don't count), penalties `{critical 25, high 15, medium 7, low 3, info 0}`. Pure and unit-tested (perfect/penalized/floored).
- **Two audiences, one data pass.** `gather_report_data` bulk-loads the project's vulns + their risk scores + compliance mappings + evidence storage-URIs (a few queries, stitched in Python — no N+1). **Executive**: security score + band, severity table, top-10 by business risk, compliance-framework coverage — business language. **Technical**: per-finding block with severity/status/CVSS(+vector)/risk rationale/compliance controls/evidence URIs — the full chain (§7 step 10). All user/finding text is HTML-escaped before it hits reportlab paragraphs (the sample data test uses `<>&` to prove escaping).
- **reportlab is imported lazily** inside the render functions, so `import`ing the reports module (service/router/CRUD, Alembic `env.py`) never requires reportlab — only the actual `render()` call does. That kept the API reloading and migrations working *before* the image rebuild.
- **Generated synchronously in the API request** (fast for typical projects), not a Celery task — POST returns the finished report with its `storage_uri`. The docstring notes it can move to a worker task later without changing the API shape if reports grow heavy. PDF bytes live in `mbs-reports` (object storage), only the pointer in Postgres (§5 trade-off).
- **API (§6):** `POST .../projects/{id}/reports` (`{type, scan_ids?}`), `GET .../reports`, `GET .../reports/{id}/download` (streams the PDF with `Content-Disposition: attachment`). `report:create` is owner/admin (an export/deliverable action); `report:read` is all roles. `reports` gets `ENABLE`+`FORCE` RLS via `project → workspace`. Storage reuses the evidence-store's boto3 client/bucket helpers (no duplication).
- **Deferred:** `scan_ids` currently filters nothing (report always covers the whole project — the field is stored for later per-scan scoping); HTML format (only PDF); charts/graphs (matplotlib — text tables for now); remediation text (the `remediations` table from §5 isn't built yet); moving generation to a Celery task; the dashboard security-score endpoint (§12 step 13) which will reuse `compute_security_score`.

## Step 11 Implementation Notes — Remediation Writer + FP Reducer (2026-07-27)

Two more AI Agent components (§3), both **mockable-first** like the planner/correlator (no key needed to build/test) and both **fail-soft** at the API layer (clean 503 when `ANTHROPIC_API_KEY` is unset, not a 500). Modules: `apps/api/ai_agent/{remediation_writer,fp_reducer}.py` + `ai_agent/prompts/`, plus `apps/api/modules/vulnerabilities/{remediation_models,ai_service}.py`. Migration `023ad0eb9823` (remediations table + RLS). Verified live: GET remediation → 404 before generation; POST generate + POST fp-analysis → 503 with no key; RLS enabled+forced.

- **Remediation Writer** turns a confirmed finding into grounded fix guidance. `write(...)` passes the finding's title/severity/category/location/CVSS/description to Claude and gets back `{summary, steps[], references[]}`. Structure is enforced in code: blank steps dropped, references without a URL dropped, non-dict junk ignored. Grounding is verified by a test asserting the finding's specifics reach the prompt. `POST .../vulnerabilities/{id}/remediation` generates (gated `vulnerability:manage`; upserts one row per vuln into `remediations`); `GET .../vulnerabilities/{id}/remediation` reads it (§6 endpoint, `vulnerability:read`).
- **FP Reducer suggests, never decides (§7 step 6).** `assess(findings)` returns a per-finding `{likely_false_positive, confidence, reasoning}`. Constrained in code: assessments filtered back to the exact input ids, unknown ids dropped, invalid confidence normalized to `low`, and **every input finding the model skipped is returned as a conservative not-FP default** — so nothing is silently dropped. `POST .../vulnerabilities/fp-analysis` returns suggestions over the project's *active* findings; acting on one goes through the **existing audited** `PATCH .../status` (justification required) — the reducer never mutates status itself, which is exactly the "log, don't silently drop" guarantee.
- **remediations schema note:** `references` is a reserved SQL word, so the column is `reference_links` (exposed as `references` in the API via a Pydantic alias). One row per vuln (unique `vulnerability_id`, upserted). Stores `generated_by` (`ai`/`human`) + `model_version` + `prompt_version` for attribution. `ENABLE`+`FORCE` RLS via the 2-hop `vulnerabilities → projects.workspace_id` join; no new permissions — reuses `vulnerability:read`/`vulnerability:manage`.
...
## Phase 5 (cont.) — Workspace API keys (2026-07-28)
